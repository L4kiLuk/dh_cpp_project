...
std::vector<Coordinates> Matchfield::hint(Coordinates_t from, bool beat_only){
    ...
    //weiß schlagen
    if(actualPlayer==1 &&field[from.x][from.y]->state==false&&field[from.x][from.y]->black==false&&field[from.x-1][from.y-1]->black==true&&field[from.x-2][from.y-2]==NULL){
    ...
    //schwarz schlagen
     if(actualPlayer==0 &&field[from.x][from.y]->state==false&&field[from.x][from.y]->black==true&&field[from.x-1][from.y+1]->black==false&&field[from.x-2][from.y+2]==NULL){
        //vector.push_back(new Coordinates_t(from.x-2, from.y+2));
    }
    if(actualPlayer==0 && field[from.x][from.y]->state==false&&field[from.x][from.y]->black==true&&field[from.x+1][from.y+1]->black==false&&field[from.x+2][from.y+2]==NULL){
        //vector.push_back(new Coordinates_t(from.x-2, from.y+2));
    ...
    //schwarz normal
    if(actualPlayer==0 &&field[from.x][from.y]->state==false&&field[from.x][from.y]->black==false&&field[from.x+1][from.y-1]==NULL){
    ...
    //weiß normal
    if(actualPlayer==1 &&field[from.x][from.y]->state==false&&field[from.x][from.y]->black==true&&field[from.x+1][from.y+1]==NULL){
        //vector.push_back(new Coordinates_t(from.x+1, from.y+1));
    }
    if(actualPlayer==1 &&field[from.x][from.y]->state==false&&field[from.x][from.y]->black==true&&field[from.x-1][from.y+1]==NULL){
        //vector.push_back(new Coordinates_t(from.x-1, from.y+1));
